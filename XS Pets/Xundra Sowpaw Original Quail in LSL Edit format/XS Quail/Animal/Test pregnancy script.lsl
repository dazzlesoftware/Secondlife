// INCLUDE //
// XS_Quail script by Xundra Snowpaw
...
default
{
    state_entry()
    {
        llMessageLinked(LINK_SET,LINK_SEX,"0","");       // make it into a female
        llSleep(1);
        
        llMessageLinked(LINK_SET,LINK_PUT_AGE,"8","");       // 8 days old
        llSleep(1);
        
        llMessageLinked(LINK_SET,LINK_SIGNAL_ELIGIBLE,"","");       // signal I am eligible
        llSleep(1);
        
        llMessageLinked(LINK_SET,LINK_MALE_ON_THE_WAY,"","");       // male on the way
        llSleep(1);
        
        
        integer times = llGetUnixTime() - 10;       // make an egg in 10 seconds
        
        llMessageLinked(LINK_SET,LINK_PREGNANCY_TIME,(string) times,"");      // set preg time to 1
        
        
        llMessageLinked(LINK_SET,LINK_PREGNANT,"8","");       // pregnant
        llSleep(1);
